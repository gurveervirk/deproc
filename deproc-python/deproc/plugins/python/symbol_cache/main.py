...
class PythonSymbolCache(SymbolCache[cache_key, cache_value]):
    ...
    def get(
        self, module_fqn: module_fqn, symbol_name: symbol_name
    ) -> cache_value | None:
    ...
    def add_cache_keys_for_module(
        self, module_fqn: module_fqn, keys: set[cache_key]
    ) -> None:
        for key in keys:
    ...
    def add_modules_for_cache_key(
        self, key: cache_key, modules: set[module_fqn]
    ) -> None:
        for mod_fqn in modules:
            self._link(mod_fqn, key)
