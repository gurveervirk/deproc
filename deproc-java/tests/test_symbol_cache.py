...
class TestSymbolCache:
    ...
    def test_set_and_get(self):
        self.cache.set("com.example.Foo", "List", {"id_1"}, {"id_2"})
    ...
    def test_cache_multiple_entries(self):
        ...
        self.cache.set("cu_b", "sym2", {"b"}, set())
        assert self.cache.get("cu_a", "sym1") == ({"a"}, set())
        assert self.cache.get("cu_b", "sym2") == ({"b"}, set())

    def test_input_lists_normalized_to_sets(self):
        self.cache.set("cu_a", "sym1", ["a", "b"], ["c"])
    ...
    def test_clear_compilation_unit(self):
        ...
        assert self.cache.get("cu_b", "sym3") == ({"c"}, set())
